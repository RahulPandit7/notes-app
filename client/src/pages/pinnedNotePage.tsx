import { useDeleteNoteMutation, useGetOwnNotesQuery, useTogglePinnedMutation } from "@/store/api/noteApi";
import { NoteCard } from "@/components/NoteCard";
import AddNoteForm from "@/components/AddNoteForm";
import type { Note } from "@/types/note";
import { useOutletContext } from "react-router-dom";

export default function PinnedNotePage() {
        ...
        <div className="container mx-auto p-8">
            <div className={`grid gap-8 items-start ${showAddForm || editingNote ? "grid-cols-1 lg:grid-cols-1" : "grid-cols-1"}`}>
                ...
                <div className={showAddForm || editingNote ? "lg:col-span-1" : "w-full"}>
                    <h2 className="text-2xl font-semibold mb-6">Pinned Notes</h2>
                    {pinnedNotes.length === 0 ? (
                        <p className="text-muted-foreground text-center py-12">No pinned notes.</p>
                    ) : (
